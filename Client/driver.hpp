...
#define IOCTL_PRINT_MESSAGE CTL_CODE(FILE_DEVICE_UNKNOWN, 0x18EA, METHOD_BUFFERED, FILE_SPECIAL_ACCESS) // where our msg came
constexpr DWORD init_code  = CTL_CODE(FILE_DEVICE_UNKNOWN, 0x18EB, METHOD_BUFFERED, FILE_SPECIAL_ACCESS); //custom io control code for setting g_target_process by target process id
constexpr DWORD read_code  = CTL_CODE(FILE_DEVICE_UNKNOWN, 0x18EC, METHOD_BUFFERED, FILE_SPECIAL_ACCESS); //custom io control code for reading memory
constexpr DWORD write_code = CTL_CODE(FILE_DEVICE_UNKNOWN, 0x18ED, METHOD_BUFFERED, FILE_SPECIAL_ACCESS); //custom io control code for writing memory
...
public:
    ...
    template<typename T> T RPM(const UINT64 address) {
        ...
        if (io_info.return_size != sizeof(T))
            throw 0xEA71CE; //partial read // EATRICE
        return read_data;
    }
